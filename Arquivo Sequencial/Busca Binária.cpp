...
//----------------------------------------------------------------------------------------------------
	...
	void buscaBinaria(Dados Busc[],int tam){
	    int Fim=tam, Inicio=0, Meio = (Inicio+Fim)/2, Proc;

	    cout << "\tQual Registro procura: ";
	    cin >> Proc;

	    while(Inicio < Fim && Busc[Meio].Codigo != Proc){
			if(Proc > Busc[Meio].Codigo){
				Inicio = Meio+1;
	    	}else{
				if(Proc < Busc[Meio].Codigo)
	        	Fim = Meio-1;
	    ...
	    if(Proc == REGISTRO[Meio].Codigo){
	        cout << "\nCodigo..: " << REGISTRO[Meio].Codigo;
	        cout << "\nNome....: " << REGISTRO[Meio].Nome;
	        cout << "\nEndereco: " << REGISTRO[Meio].Endere;
	        cout << "\nCidade..: " << REGISTRO[Meio].Cidade;
	        cout << "\nUF......: " << REGISTRO[Meio].UF;
	        cout << "\n\n";
	    }
	     else {
