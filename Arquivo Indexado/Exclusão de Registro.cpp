...
//--------------------------EXCLUSÃO---------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
int ExclusaoRegistro(Index Busc[],Dados registro[],int tam){
    int del, Fim=tam,Inicio=0, Meio = (Inicio+Fim)/2;

    cout << "\nQual Arquivos ira apagar: ";
    cin >> del;

        while(Inicio < Fim && Busc[Meio].Cod != del){
            if(del > Busc[Meio].Cod){
                Inicio = Meio+1;
            }else{
                  if(del < Busc[Meio].Cod)
                  Fim = Meio-1;
    ...
    if((registro[Meio].Codigo == del) && (registro[Meio].status == false)){
     registro[Meio].status = true;
     return tam-1;
    }else{
